...
\begin{frame}[t]{Echo-aware Applications \hfill\faBook}
    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{block}{\textbf{What?}}
            ...
            \vspace{-2mm}
            \begin{itemize}
                \item \only<-5>{Sound Source Separation}\only<6->{\alert{Sound Source Separation}}
                \\{\footnotesize\cite{leglaive2016multichannel}}
                \item \only<-5>{Speech Enhancement}\only<6->{\alert{Speech Enhancement}}
        ...
        \begin{block}{\textbf{Where?}}
            ...
            \vspace{-2mm}
            \begin{itemize}
                \item \only<-5>{Sound Source Localization}\only<6->{\alert{Sound Source Localization}}
                \\{\footnotesize\cite{ribeiro2010turning,jensen2019method}}
                \item \only<-5>{Room Geometry Estimation}\only<6->{\alert{Room Geometry Estimation}}
    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{block}{SSL with more microphones}
            \begin{itemize}
                ...
                \\\addendum{known limitation, but good in practice}
            \end{itemize}
        \end{block}
        \column{0.38\textwidth}
